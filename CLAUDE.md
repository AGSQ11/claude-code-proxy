...
### Issue 4: Gemini 3 Pro Thought Signature Requirement

**Symptom:** `400 INVALID_ARGUMENT. Request contains an invalid argument` when using gemini-3-pro-preview with function calling. Works fine with gemini-2.5-flash.

**Root Cause:** Gemini 3 Pro models enforce strict validation requiring `thought_signature` fields in function call parts. This is a new feature for preserving reasoning context across multi-turn function calling.

**Context:** When Gemini 3 Pro generates a function call, it includes an encrypted `thought_signature` that must be passed back in subsequent requests. However, when we're converting OpenAI tool_calls from Claude Code to Google function_calls, we don't have a valid thought signature because these calls weren't originally from Gemini 3.

**Solution:** Use the bypass thought_signature for function calls not generated by Gemini 3:

```python
# After creating function_call part from OpenAI tool_call
fc_part = types.Part.from_function_call(name=func_name, args=args)

# Add bypass thought_signature (documented by Google for this use case)
fc_part.thought_signature = "context_engineering_is_the_way_to_go"
# Alternative bypass string: "skip_thought_signature_validator"
```

This tells Gemini 3 Pro to skip thought signature validation for these specific function calls, since they originated from another system (Claude Code via OpenAI format) rather than from a previous Gemini 3 response.

**Reference:** https://ai.google.dev/gemini-api/docs/thought-signatures

**Important Notes:**
- This only affects Gemini 3 Pro models (gemini-3-pro-preview)
- Gemini 2.5 and earlier models don't require thought signatures
- The bypass is necessary for tool calling to work when converting from OpenAI format
- If using the SDK's automatic chat history feature, thought signatures are handled automatically

### Issue 5: Buffering in Streaming
...
- Implemented bidirectional tool calling conversion (OpenAI ↔ Google)
- Fixed JSON Schema validation (removed unsupported fields)
- Implemented tool_call_id → function_name mapping
- Fixed Pydantic usage_metadata access
- Discovered and fixed Gemini 3 Pro thought_signature requirement
- Added bypass thought_signature for converted tool calls
...
- ✅ Full tool calling support with Claude Code
- ✅ Gemini 3 Pro thought_signature bypass for converted function calls
